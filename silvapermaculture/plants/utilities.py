...
def save_plant_picture(form_plantPic):
    ...
    """
    random_hex_image = secrets.token_hex(8).decode("utf-8", "ignore")
    _, f_ext = os.path.splitext(form_plantPic.filename)
    picture_fn = random_hex_image + f_ext
    picture_path = os.path.join(current_app.root_path, 'static/img/plants', picture_fn) #Saving the new plantPic to the specified folder.
    scale_image = (286, 180)
